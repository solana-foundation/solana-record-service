...
impl<'info> TryFrom<&'info [AccountInfo]> for UpdateClassAccounts<'info> {
    ...
    fn try_from(accounts: &'info [AccountInfo]) -> Result<Self, Self::Error> {
        ...
        class_state.validate_authority(authority)?;
...
/// Minimum length of instruction data required for UpdateClassMetadata
/// 
/// This constant represents the minimum number of bytes needed in the instruction
/// data, which includes:
/// * 1 byte for the metadata length prefix
...
impl<'info> TryFrom<Context<'info>> for UpdateClassMetadata<'info> {
    ...
    fn try_from(ctx: Context<'info>) -> Result<Self, Self::Error> {
        ...
        let metadata = std::str::from_utf8(&ctx.data[UPDATE_CLASS_METADATA_MIN_LENGTH..])
            .map_err(|_| ProgramError::InvalidInstructionData)?;

        // Validate metadata length
        if metadata.len() > 1024 { // Arbitrary limit, adjust as needed
            return Err(ProgramError::InvalidInstructionData);
        }
